...
class DStar:
    ...
    def process_state(self):
        ...
        if k_old < node.h:
            for Y in neighbors:
                if (Y.h <= k_old) and (node.h > Y.h + self.cost(Y,node)):
                    node.parent = Y
                    node.h = Y.h + self.cost(Y,node)
        # If node k is the same as h (LOWER)
        if k_old == node.h:
            for Y in neighbors:
                if (Y.tag == "NEW") or ((Y.parent == node) and (Y.h != node.h + self.cost(node,Y))) or ((Y.parent != node) and (Y.h > node.h + self.cost(node,Y))):
                    Y.parent = node
        ...
        # Else node k is smaller than h (RASIE)
            ...
            for Y in neighbors:
                if (Y.tag == "NEW") or ((Y.parent == node) and (Y.h != node.h + self.cost(node,Y))):
                    ...
                    if((Y.parent != node) and (Y.h > node.h + self.cost(node,Y))):
                        self.insert(node, node.h)
                    else:
                        if((Y.parent != node) and (node.h > Y.h + self.cost(Y,node)) and (Y.tag == "CLOSED") and Y.h > k_old):
                            self.insert(Y,Y.h)
        #### TODO END ####
    ...
    def run(self):
        ...
        k_min = math.inf
        while(k_min != -1) and (curr_node.tag != "CLOSED"):
            k_min = self.process_state()
        ...
        while (curr_node_dy != self.goal):
            ...
            curr_node_dy = curr_node_dy.parent
        #### TODO END ####
